...
public class CartService {
    ...
    public Order checkout(User user, com.cnx.backend.dto.CheckoutRequest request) {
        ...
        order.setStatus(com.cnx.backend.entity.OrderStatus.PENDING);
        ...
        if (request.getPickDate() != null) {
            order.setPickDate(java.time.LocalDate.parse(request.getPickDate()));
        }
        if (request.getDeliveryDate() != null) {
            order.setDeliveryDate(java.time.LocalDate.parse(request.getDeliveryDate()));
        }

        // invoice date is generated by the server when the order is created
        order.setInvoiceDate(java.time.LocalDateTime.now());
    ...
    public java.util.List<com.cnx.backend.dto.OrderSummaryDto> getOrderSummaries(User user) {
        return orderRepository.findByUserId(user.getId()).stream().map(order -> {
            ...
            dto.setStatus(order.getStatus() == null ? com.cnx.backend.entity.OrderStatus.PENDING.name() : order.getStatus().name());
            dto.setPickDate(order.getPickDate() == null ? null : order.getPickDate().toString());
            dto.setDeliveryDate(order.getDeliveryDate() == null ? null : order.getDeliveryDate().toString());
            dto.setInvoiceDate(order.getInvoiceDate() == null ? null : order.getInvoiceDate().toString());
    ...
    /**
     * Update order status (e.g., confirmed/out for delivery/out of stock)
     */
    public com.cnx.backend.entity.Order updateOrderStatus(User user, Long orderId, com.cnx.backend.entity.OrderStatus newStatus) {
        com.cnx.backend.entity.Order order = orderRepository.findById(orderId)
                .orElseThrow(() -> new RuntimeException("Order not found"));

        if (!order.getUser().getId().equals(user.getId())) {
            throw new RuntimeException("Not authorized to modify this order");
        }
